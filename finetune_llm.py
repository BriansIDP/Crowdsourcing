...
from dataloader import SupervisedDataset, collate_sft_fn
...
def main(rank, args, world_size):
    ...
    ## Initialise data
    tokenizer = AutoTokenizer.from_pretrained(args.model_path, cache_dir="/scratch/NeurowaveEval/leaderboard/bot/cache")
    ...
    valid_dataloader = DataLoader(
        ...
        shuffle=False,
        collate_fn=collate_sft_fn,
        # sampler=DistributedSampler(valdata),
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            logging(f"Epoch {epoch} | Validation PPL: {val_ppl} | Validation Acc: {val_acc/world_size} | Learning rate: {current_lr}", args.logfile)
...
def train_one_epoch(
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        #     inputs["attention_mask"] = (inputs["input_ids"] != -1)
        #     inputs["input_ids"] = (inputs["input_ids"] == 0) * 1 + inputs["input_ids"]
        ...
        if (i + 1) % args.log_interval == 0 and accelerator.is_main_process:
            ...
            logging(f"Epoch {epoch} | Batch {i+1}/{trainsize} | PPL: {PPL} | time {elasped_time}", args.logfile)
    return model


def eval_one_epoch(args, model, valid_dataloader, criterion, epoch):
    total_tokens = 0.
    total_loss = 0.
    for i, batch in enumerate(valid_dataloader):
        inputs, labels = batch
        # Otherwise outputs nan
        # if "llama-2" in args.model_path:
        #     # inputs["attention_mask"] = (inputs["input_ids"] != -1)
        #     inputs["input_ids"] = (inputs["input_ids"] == 0) * 1 + inputs["input_ids"]
        with torch.cuda.amp.autocast():
