...
axs[0, 3].hist(data[:, 3], bins=nbins)
axs[0, 3].set_xlim(-limits, limits)
# axs[1, 0].set_title("Beluga")
axs[0, 3].set_title("Zephyr")
# axs[1, 0].set_title("Mean=1.0, Var=2.0, Acc=72.7, Groundtruth")

# data = np.load("outputs/zhat_2_2_gt.npy")
axs[0, 4].hist(data[:, 4], bins=nbins)
axs[0, 4].set_xlim(-limits, limits)
# axs[1, 1].set_title("Zephyr")
axs[0, 4].set_title("Starling")
...
plt.savefig("/Users/briansun/Documents/PAPER/Crowdsourcing/plots/histogram_haludial.pdf", format="pdf")
plt.show()
