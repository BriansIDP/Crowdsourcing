...
class FinetuneLM:
    def __init__(self, model, train_dataloader, val_dataloader,
                model_dir: str, lr: float=0.001, weight_decay: float=1e-5, 
                gradient_accumulation_steps: int=1, num_warmup_steps: float=0.03,
                max_grad_steps: int=5000, lr_scheduler_type: str='cosine',
                log_interval: int=100, eval_interval: int=500, patience: int=2, 
                loss_fn_type='bce', need_ests: bool=False) -> None:
        self.model = model
        self.train_dataloader = train_dataloader
        self.val_dataloader = val_dataloader
        self.model_dir = model_dir
